...
/**
 * @DO 评论情感正负判断
 *    ???本地模式跑了3分钟 (第三次  21:51-21:55)(21:57-22:01)
 *
...
object EmotionAnalysis {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val emotionDF: DataFrame = commentRDD
      .map(
        line => {
          val strs: Array[String] = line.split("\t")
          val classifier = trainOrGetMyModel() // 获取分类器
          (strs(1), classifier.classify(strs(2))) // 预测情感
        }
      )
      .groupByKey()
      .map {
    ...
    val weiboDF: DataFrame = HdfsToSpark.getSequenceFileFromFolder(spark.sparkContext, MyConfig.WEIBO_DATA_PATH)
      .map(
        line => {
          ...
          (strs(0), strs(1), strs(6), strs(8), strs(9), strs(10), strs(11).toDouble, strs(12).toDouble, strs(13).toDouble)
        }
      )
      .toDF("title", "messageId", "createTime", "userName", "messageUrl", "text", "repostsCount", "commentsCount", "attitudesCount")
